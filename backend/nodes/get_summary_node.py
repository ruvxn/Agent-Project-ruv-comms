...
def get_summary_node(state: GraphState) -> GraphState:
    ...
    for single_page_pdf_text in state.pdf.chunked_pdf_text:
        summaries.append(single_chunk_summary(
            clean_text(single_page_pdf_text.chunk)))

    if summaries:
        summaries_string = " ".join(summaries)
        state.pdf.final_summary = single_chunk_summary(summaries_string)
        insert_pdf_summary(state)
    else:
        state.pdf.final_summary = "No content to summarize."
